...
@cache_result(
    ttl=300,
    user_id="user_history_{user_id_}_conn_{conn_id}_limit_{limit}_offset_{offset}",
)
...
def fetch_audit_logs(
    ...
    logs = (
        query.order_by(desc(QueryHistory.executed_at)).offset(offset).limit(limit).all()
    )
