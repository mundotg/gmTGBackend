...
@cache_result(ttl=300, user_id="tables_with_count_{user_id_}_{connection_id}")
async def get_tables_with_count_cached(
    connection_id: int,
    user_id_: int,
    db: Session,
) -> List[Dict[str, Any]]:
    return get_table_names_with_count(connection_id, user_id_, db)


@cache_result(ttl=600, user_id="tables_names_{user_id_}")
async def get_tables_names_cached(
    connection_id: int,
    user_id_: int,
    db: Session,
) -> List[str]:
    return get_table_names(connection_id, user_id_, db)


@cache_result(ttl=600, user_id="fields_bulk_{parn_user_id}")
async def get_fields_info_bulk_cached_wrapper(
    connection_id: int,
    parn_user_id: int,
    ...
    return service_get_fields_info_bulk_cached(
        ...
        user_id=parn_user_id,
        db=db,
    )


@cache_result(ttl=600, user_id="fields_{user_id_}")
async def get_fields_info_cached_wrapper(
    connection_id: int,
    user_id_: int,
    table_name: str,
    db: Session,
...
async def get_structures_names_cached(
    ...
    return get_strutures_names(connection_id, parn_user_id, db)


@cache_result(
    ttl=180, user_id="table_count_{parn_user_id}_{table_name}_{connection_id}"
)
async def get_table_count_cached(
    ...
    return get_table_count(connection_id, table_name, db, parn_user_id)


@cache_result(ttl=1800, user_id="stats_sync_{parn_user_id}")
async def sync_connection_stats_cached(parn_user_id: int, db: Session) -> Any:
    stats = sync_connection_statistics(parn_user_id, db)
...
async def get_tables_with_count_endpoint(
    ...
    try:
        ...
        tables = await get_tables_with_count_cached(active.connection_id, user_id, db)
...
async def get_tables_endpoint(
    ...
    try:
        active = _get_active_connection_or_400(db, user_id)
        names = await get_tables_names_cached(active.connection_id, user_id, db)
...
async def get_fields_by_connection_id_endpoint(
    ...
    try:
        ...
        fields = await get_fields_info_cached_wrapper(
            connection_id=connection_id,
            user_id=user_id,
            table_name=table_name,
            db=db,
...
async def get_fields_by_connection_id_and_table_names_endpoint(
    ...
    try:
        ...
        fields_by_table = await get_fields_info_bulk_cached_wrapper(
            connection_id=connection_id,
            table_names=table_names,
            parn_user_id=user_id,
            db=db,
...
async def get_table_count_endpoint(
    ...
    try:
        ...
        count = await get_table_count_cached(
            active.connection_id, table_name, db, user_id
        )
