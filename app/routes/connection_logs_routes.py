...
async def list_connection_logs(
    ...
    try:
        if page < 1 or limit < 1:
            raise HTTPException(
                status_code=400, detail="Parâmetros de paginação inválidos."
            )

        return get_connection_logs_pagination_cached(
            db, user_id, connection_id, page, limit
        )
    except Exception as e:
        log_message(
            f"❌ Erro ao listar logs: {str(e)}\n{traceback.format_exc()}", "error"
        )
