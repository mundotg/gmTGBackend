...
def list_projects_cached(db: Session, user_id: int,user_id_principal: int = Depends(get_current_user_id)):
    """Lista projetos com cache."""
    return project_service.list_projects_service(db)

@cache_result(ttl=600, user_id="user_{user_id}")
def retrieve_project_cached(db: Session, project_id: str, user_id: int,user_id_principal: int = Depends(get_current_user_id)):
...
def list_projects(db: Session = Depends(get_db), user: str = Depends(get_current_user_id_task), user_id_principal: int = Depends(get_current_user_id)):
...
def retrieve_project(project_id: str, db: Session = Depends(get_db), user: str = Depends(get_current_user_id_task), user_id_principal: int = Depends(get_current_user_id)):
...
def create_new_project(project: ProjectSchema, db: Session = Depends(get_db), user: str = Depends(get_current_user_id_task), user_id_principal: int = Depends(get_current_user_id)):
...
def update_existing_project(project_id: str, project: ProjectSchema, db: Session = Depends(get_db), user_id: int = Depends(get_current_user_id), user_id_principal: int = Depends(get_current_user_id)):
...
def delete_existing_project(project_id: str, db: Session = Depends(get_db), user: str = Depends(get_current_user_id_task), user_id_principal: int = Depends(get_current_user_id)):
