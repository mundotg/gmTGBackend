    ...
    async def generator() -> AsyncGenerator[str, None]:
        try:
            ...
            if not os.path.exists(safe_path):
                raise ValueError(
                    f"Arquivo não encontrado: {os.path.basename(safe_path)}"
                )
            ...
            msg = (
                f"🔧 Restaurando a partir do arquivo: {os.path.basename(safe_path)} ..."
            )
            channel_manager.push(ch_id, user_id, msg)
            yield sse("log", msg)
...
@router.get("/br/channel/{channel_id}/status")
...
@router.get("/br/channels/cleanup", operation_id="cleanup_check_backup")
...
@router.get("/br/health", operation_id="health_check_backup")
