...
from app.cruds.dbstatistics_crud import (
    converter_stats,
    get_statistics_by_connection_geral,
)
...
def _get_user_connection_info(user_id: int, db: Session) -> Dict[str, Any]:
    ...
    if not active:
        raise HTTPException(
            status_code=404,
            detail="Nenhuma conexão ativa encontrada. Por favor, conecte-se a um banco de dados.",
        )
    ...
    if not connection:
        raise HTTPException(
            status_code=404,
            detail="As credenciais desta conexão não foram encontradas.",
        )
...
def _get_user_engine(user_id: int, db: Session):
    ...
    if not engine:
        log_message(
            f"Engine não encontrada para user {user_id}. Tentando reativar...",
            level="warning",
        )
        ...
        if not res.get("success"):
            raise HTTPException(
                status_code=503,
                detail="Não foi possível reestabelecer a conexão com o Banco de Dados. Verifique se o servidor está online.",
            )
        engine = EngineManager.get(user_id)
...
def _safe_json_loads(json_str: str, default: Any = None) -> Any:
    try:
        ...
        log_message(
            "Aviso: Falha ao decodificar JSON recebido na URL.", level="warning"
        )
...
def get_metadata_db_cached(user_id: int, db: Session):
    ...
    stats = converter_stats(
        get_statistics_by_connection_geral(connection_info["active"].connection_id)
    )
    if not stats:
        stats = sync_connection_statistics(user_id, db)
        if not stats:
            raise HTTPException(
                status_code=500, detail="Falha ao obter estatísticas da conexão"
            )

    # Passamos o schema para filtrar tabelas do sistema massivas
    table_info = get_table_names_with_count(
        connection_info["active"].connection_id, user_id, db
    )
...
async def get_metadata_db(
    schema: Optional[str] = Query(
        None, description="Filtra tabelas por schema (ex: public, dbo)"
    ),
    refresh_cache: bool = Query(
        False, description="Força a atualização dos metadados ignorando o cache"
    ),
    ...
    try:
        ...
        return ResponseWrapper(
            ...
            data=DatabaseMetadata(
                ...
                tableNames=[
                    TableInfo(name=t["name"], rowcount=t.get("rowcount", 0))
                    for t in result["tableNames"]
                ],
            ),
        )
        ...
        raise HTTPException(
            status_code=500, detail="Erro interno ao carregar metadados do banco."
        )
...
async def get_metadata_fields(
    ...
    if not table_name or not table_name.strip():
        raise HTTPException(
            status_code=400, detail="Nome da tabela não pode estar vazio."
        )
...
async def get_row_data_by_index(
    index: int,
    table_name: str = Query(..., description="Nome da tabela"),
    schema: Optional[str] = Query(None, description="Schema da tabela"),
    primary_key_field: str = Query(..., description="Campo identificador (PK)"),
    primary_key_value: str = Query(
        None, description="Valor da chave primária para busca direta"
    ),
    ...
    try:
        if primary_key_value and index != 0:
            raise HTTPException(
                status_code=400,
                detail="Informe 'primary_key_value' OU 'index', não ambos simultaneamente.",
            )
        ...
        try:
            ...
            raise HTTPException(
                status_code=422, detail="Formato de 'order_by' inválido."
            )
        ...
        if not row_data:
            raise HTTPException(
                status_code=404,
                detail="O registro solicitado não foi encontrado ou foi excluído.",
            )
        ...
        raise HTTPException(
            status_code=500,
            detail="Erro ao buscar o registro detalhado no banco de dados.",
        )
...
async def get_row_data_query_by_index(
    index: int = Path(
        ..., ge=0, description="O índice (offset) exato da linha desejada"
    ),
    ...
    try:
        ...
        log_message(
            f"❌ Erro em get_row_data_query_by_index: {e}\n{traceback.format_exc()}",
            level="error",
        )
        raise HTTPException(
            status_code=500,
