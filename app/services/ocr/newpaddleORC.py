...
def group_text_by_lines(texts, boxes, y_threshold=15):
    ...
    for text, box in zip(texts, boxes):
        ...
        items.append({"text": text, "x": x_min, "y": y_center})
    ...
    for item in items:
        ...
        if not placed:
            lines.append({"y": item["y"], "words": [item]})
...
def analyze_image(image_bytes):
    try:
        ...
        return {"text": full_text, "lines": lines, "file": None}
        ...
        return {"error": type(e).__name__, "message": str(e), "traceback": error_trace}
