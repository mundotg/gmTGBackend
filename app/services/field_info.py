...
from app.cruds.dbstructure_crud import (
    create_db_field,
    create_db_structure,
    delete_structure_by_name,
    get_db_structures_by_conn_id_and_table,
    get_fields_by_structure,
)
...
from app.services.fields_estruture import (
    _is_system_field_from_column,
    obter_schema_do_engine,
)
...
def sincronizar_metadados_da_tabela(db: Session, table_name: str, user_id: int) -> dict:
    try:
        ...
        fields_table: List[DBField] = buscar_ou_criar_campos_tabela(
            db, structure, engine, str(db_type)
        )

        # 5. Busca valores ENUM por coluna
        enum_map: Dict[str, List[str]] = _fetch_enum_values(
            db, fields_table, engine, structure, db_type
        )

        # 6. Processa e sincroniza campos ENUM
        resposta_colunas = processar_enum_fields(
            columns=fields_table, enum_map=enum_map
        )
        ...
        db.rollback()
        log_message(
            f"❌ Erro de banco de dados ao sincronizar a tabela '{table_name}': {str(e)}{traceback.format_exc()}",
            "error",
        )
        raise RuntimeError(
            f"❌ Erro de banco de dados ao sincronizar a tabela '{table_name}': {str(e)}"
        )

    except Exception as e:
        db.rollback()
        log_message(
            f"❌ Erro inesperado ao sincronizar a tabela '{table_name}': {str(e)}{traceback.format_exc()}",
            "error",
        )
        raise RuntimeError(
            f"❌ Erro inesperado ao sincronizar a tabela '{table_name}': {str(e)}"
        )
...
def get_fields_of_tables_bulk(
    ...
    for table_name in table_names:
        try:
            ...
            fields_table: List[DBField] = buscar_ou_criar_campos_tabela(
                db=db, structure=structure, engine=engine, db_type=str(db_type)
            )
...
def processar_enum_fields(
    columns: List[DBField], enum_map: Dict[str, List[str]]
) -> List[CampoDetalhado]:
    """
...
def buscar_ou_criar_campos_tabela(
    ...
    try:
        columns, inspector = safe_get_columns(
            engine, structure.table_name, schema=structure.schema_name
        )
    ...
    try:
        ...
        unique_constraints = []
        log_message(
            "⚠️ Este dialeto não implementa 'get_unique_constraints'. Continuando sem verificar unicidade...",
            level="warning",
        )
    ...
    foreign_keys_info = inspector.get_foreign_keys(
        structure.table_name, schema=structure.schema_name
    )
    ...
    for column in columns:
        ...
        field_in = DBFieldCreate(
            ...
            referenced_field=(
                relation.get("fieldTabelaRelacao") if is_foreign_key else None
            ),
        ...
        field_create = create_db_field(
            db=db, field_in=field_in, structure_id=structure.id
        )
