...
def build_type_string(field: FieldDDLRequest, db_type: str) -> str:
    ...
    # 🔥 1. ENUM — só MySQL/MariaDB têm ENUM nativo. Nos outros SGBD guarda-se
    # como texto e a restrição de valores vem por CHECK (ver _enum_check_clause).
    if type_str == "ENUM" and getattr(field, "enum_values", None):
        if db_type in ["mysql", "mariadb"]:
            vals = ", ".join(_q(v) for v in field.enum_values)
            type_str = f"ENUM({vals})"
        else:
            maxlen = max((len(str(v)) for v in field.enum_values), default=50)
            type_str = f"VARCHAR({max(maxlen, 1)})"
    ...
    # 🔥 2. UNSIGNED — só MySQL/MariaDB suportam nativamente em tipos numéricos.
...
def _enum_check_clause(db_type: str, field: FieldDDLRequest, safe_col: str) -> str:
    """CHECK (col IN (...)) para simular ENUM onde não é nativo."""
    if str(field.type).upper().strip() != "ENUM":
        return ""
    if db_type in ["mysql", "mariadb"]:
        return ""  # ENUM nativo
    vals = getattr(field, "enum_values", None)
    if not vals:
        return ""
    joined = ", ".join(_q(v) for v in vals)
    return f"CHECK ({safe_col} IN ({joined}))"


def _pg_serial_override(type_str: str) -> str:
    """Mapeia o tipo inteiro para o SERIAL correspondente (auto-increment PG)."""
    t = type_str.upper()
...
def _identity_tokens(db_type: str, field: FieldDDLRequest, type_str: str) -> "tuple[str, str]":
    """
    Auto-increment/identidade por SGBD. Devolve (type_override, extra_token).

    - PostgreSQL: substitui o tipo por SERIAL/BIGSERIAL/SMALLSERIAL.
    - MySQL/MariaDB: AUTO_INCREMENT.
    - SQL Server: IDENTITY(1,1).
    - Oracle: GENERATED BY DEFAULT AS IDENTITY.
    - SQLite: INTEGER (o AUTOINCREMENT liga-se à PK — tratado no compositor).
    """
    if not getattr(field, "is_auto_increment", False):
        return type_str, ""
    if db_type in ["postgresql", "postgres"]:
        return _pg_serial_override(type_str), ""
    if db_type in ["mysql", "mariadb"]:
        return type_str, "AUTO_INCREMENT"
    if db_type in ["mssql", "sqlserver"]:
        return type_str, "IDENTITY(1,1)"
    if db_type == "oracle":
        return type_str, "GENERATED BY DEFAULT AS IDENTITY"
    if db_type == "sqlite":
        return "INTEGER", ""
    return type_str, ""


def _compose_add_column_body(
    db_type: str,
    ...
    """
    Monta o corpo do `ADD COLUMN <col> ...` respeitando a ordem/gramática de
    cada SGBD, incluindo PRIMARY KEY, auto-increment/identidade e o CHECK do
    ENUM emulado. É `is_primary_key` que faltava por completo no DDL.
    """
    is_ai = bool(getattr(field, "is_auto_increment", False))
    is_pk = bool(getattr(field, "is_primary_key", False))
    pk_str = "PRIMARY KEY" if is_pk else ""
    check_str = _enum_check_clause(db_type, field, safe_col)

    ttype, ident = _identity_tokens(db_type, field, type_str)

    # SERIAL (PG) já implica sequência/NOT NULL → não repetir DEFAULT/UNSIGNED.
    pg_serial = db_type in ["postgresql", "postgres"] and is_ai
    eff_default = "" if pg_serial else default_str

    if db_type in ["postgresql", "postgres"]:
        parts = [ttype, eff_default, ("" if pg_serial else null_str), unique_str, pk_str, check_str]
    elif db_type in ["mysql", "mariadb"]:
        # AUTO_INCREMENT exige ser chave → força PK quando auto-increment.
        pk_eff = "PRIMARY KEY" if (is_pk or is_ai) else ""
        parts = [ttype, null_str, default_str, ident, unique_str, pk_eff, check_str]
    elif db_type in ["mssql", "sqlserver"]:
        parts = [ttype, ident, null_str, default_str, unique_str, pk_str, check_str]
    elif db_type == "oracle":
        parts = [ttype, eff_default, ident, null_str, unique_str, pk_str, check_str]
    elif db_type == "sqlite":
        if is_ai and is_pk:
            # Único modo suportado: INTEGER PRIMARY KEY AUTOINCREMENT.
            parts = ["INTEGER", "PRIMARY KEY AUTOINCREMENT", check_str]
        else:
            parts = [ttype, default_str, null_str, unique_str, pk_str, check_str]
    else:
        parts = [ttype, default_str, null_str, unique_str, pk_str, check_str]

    body = " ".join(p for p in parts if p and p.strip())
    return f"{safe_col} {body}"
...
# ============================================================
# 🍃 MongoDB — "colunas" são campos dos documentos
# ============================================================


def _is_mongo(engine, db_type: Optional[str]) -> bool:
    return is_mongo(engine) or (db_type or "").lower() == "mongodb"


def _mongo_collection(engine, structure_name: str):
    schema, collection = _split_schema_table(structure_name)
...
def _mongo_field_op(
    ...
    *,
    ...
    """Aplica um update a TODOS os documentos da coleção e audita.

    O Mongo não tem schema: um campo só "existe" nos documentos que o têm. Por
    isso adicionar/remover/renomear uma coluna é um `update_many` — e o número
    de documentos afetados fica registado na auditoria.
    """
    started = _utcnow()
    coll, nome = _mongo_collection(engine, structure_name)

    resultado = coll.update_many({}, update)
    afetados = int(getattr(resultado, "modified_count", 0) or 0)
    ...
    _audit_write(
        ...
        column=column,
        sql=f"db.{nome}.updateMany({{}}, {resumo})",
        status="success",
        started_at=started,
        extra={"documentos_afetados": afetados},
    )
    return afetados
...
def execute_add_column(
    ...
    *,
    ...
    # NoSQL: adicionar campo = pô-lo em todos os documentos (com o default, ou
    # null). Sem isto o campo não apareceria na grelha da app.
    if _is_mongo(engine, connection_model.type):
        valor = getattr(field, "default", None)
        _mongo_field_op(
            db,
            engine,
            ...
            column=field.name,
            update={"$set": {field.name: valor}},
            resumo=f"{{$set: {{{field.name}: {valor!r}}}}}",
        )
        return
    ...
    # Corpo da coluna (com PK, auto-increment/identidade e CHECK do ENUM) por SGBD.
    col_body = _compose_add_column_body(
        db_type, field, safe_col, type_str, null_str, default_str, unique_str
    )

    if db_type in ["postgresql", "postgres", "mysql", "mariadb", "mssql", "sqlserver"]:
        add_kw = "ADD COLUMN" if db_type in ["postgresql", "postgres", "mysql", "mariadb"] else "ADD"
        sql = f"ALTER TABLE {safe_table} {add_kw} {col_body};"
    elif db_type == "oracle":
        sql = f"ALTER TABLE {safe_table} ADD ({col_body})"
    elif db_type == "sqlite":
        # SQLite: NOT NULL sem default só é aceite se houver PK/AUTOINCREMENT.
        if (not field.is_nullable) and not field.default_value and not (
            field.is_primary_key and field.is_auto_increment
        ):
            raise ValueError(
                "SQLite exige default_value para colunas NOT NULL ao usar ADD COLUMN."
            )
        sql = f"ALTER TABLE {safe_table} ADD COLUMN {col_body};"
    else:
...
def execute_alter_column(
    ...
    *,
    ...
    # NoSQL: só o rename tem equivalente — tipo, nullable e unique não são
    # propriedades do campo no Mongo.
    if _is_mongo(engine, connection_model.type):
        original_name = field.original_name or field.name
        if original_name == field.name:
            log_message(
                f"⚠️ MongoDB: alteração de '{field.name}' sem rename — tipo e "
                "restrições não se aplicam a coleções; nada a fazer.",
                "warning",
            )
            return
        ...
        _mongo_field_op(
            ...
            structure_name=table_name,
            column=field.name,
            update={"$rename": {original_name: field.name}},
            resumo=f"{{$rename: {{{original_name}: {field.name}}}}}",
        )
        return
...
def execute_drop_column(
    ...
    *,
    ...
    # NoSQL: remover campo = $unset em todos os documentos.
    if _is_mongo(engine, dialect):
        db_type = "mongodb"
        _mongo_field_op(
            db,
            engine,
            ...
            column=column_name,
            update={"$unset": {column_name: ""}},
            resumo=f"{{$unset: {{{column_name}: ''}}}}",
        )
    else:
