...
class FieldDefinition(BaseModel):
    ...
    original_name: Optional[str] = Field(
        None, description="Se preenchido, indica edição. Se None, é criação."
    )
    nome: str
    tipo: str
...
class QueryChannelManager:
    ...
    def cleanup_expired_channels(self) -> int:
        expired = [cid for cid, ch in self.channels.items() if self._is_expired(ch)]
        for cid in expired:
...
async def register_field_task(
    field_data: FieldDefinition = Body(...), user_id: int = Depends(get_current_user_id)
):
    """
    ...
    """
    try:
        channel_id = channel_manager.create_channel(
            user_id=user_id, payload=field_data.dict()
        )
        return {
            "success": True,
            "channel_id": channel_id,
            "message": "Task registrada. Inicie o stream.",
        }
    except Exception as e:
        log_message(f"Erro ao registrar task de field: {e}", level="error")
        raise HTTPException(
            status_code=500, detail="Não foi possível registrar a operação."
        )
...
@router.get("/health", operation_id="health_check_main")
