...
DB_QUERIES: Dict[str, Dict[str, Callable]] = {
    "postgresql": {
        "size": lambda conn: safe_scalar(conn, text("SELECT pg_database_size(current_database())"), 0),
        "rows": lambda conn: safe_scalar(conn, text("SELECT COALESCE(SUM(n_live_tup),0) FROM pg_stat_user_tables"), 0),
        "tx": lambda conn: safe_scalar(conn, text("SELECT COUNT(*) FROM pg_stat_activity WHERE state='active'"), 0),
        "deadlocks": lambda conn: safe_scalar(conn, text("SELECT COALESCE(SUM(deadlocks),0) FROM pg_stat_database"), 0),
    },

    "mysql": {
        "size": lambda conn: safe_scalar(conn, text("""
            SELECT SUM(data_length + index_length)
            FROM information_schema.tables
            WHERE table_schema = DATABASE()
        """), 0),

        "rows": lambda conn: safe_scalar(conn, text("""
            SELECT SUM(table_rows)
            FROM information_schema.tables
            WHERE table_schema = DATABASE()
        """), 0),

        "tx": lambda conn: safe_scalar(conn, text("SELECT COUNT(*) FROM information_schema.processlist"), 0),
    ...
    "sqlserver": {
        ...
        "tx": lambda conn: safe_scalar(conn, text("SELECT COUNT(*) FROM sys.dm_exec_requests"), 0),

        "deadlocks": lambda conn: safe_scalar(conn, text("""
            SELECT cntr_value
    ...
    "oracle": {
        "size": lambda conn: safe_scalar(conn, text("SELECT SUM(bytes) FROM dba_segments"), 0),
        "rows": lambda conn: 0,
        "tx": lambda conn: 0,
        "deadlocks": lambda conn: 0,
    },
...
# CACHEABLE WRAPPER
# =========================
@cache_result(ttl=120, user_id="user_db_metrics_{user_id}_conn_{conn_id}")
def get_metrics_cached(*, user_id: int, conn_id: int, engine: Engine, conn_info: DBConnection):
